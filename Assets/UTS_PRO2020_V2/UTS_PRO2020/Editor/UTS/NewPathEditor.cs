    ...
    public override void OnInspectorGUI()
    {
    	...
    	if(GUILayout.Button("Finish"))
    	{
    		if(!_NewPath.errors)
    		{
                ...
                {
                    wp = _NewPath.gameObject.AddComponent<PeopleWalkPath>();
                }

                else if(_NewPath.PathType == PathType.VehiclePath)
                {
                    wp = _NewPath.gameObject.AddComponent<CarWalkPath>();
                }
                else if(_NewPath.PathType == PathType.AudiencePath)
                {
                    wp = _NewPath.gameObject.AddComponent<AudiencePath>();
                }
                else
                {
                    wp = _NewPath.gameObject.AddComponent<BcycleGyroPath>();
                }
